...
class GladysPlugin(octoprint.plugin.TemplatePlugin,
	...
	def __init__(self):
        ...
        def initialize(self):
                ...
                self.plug = self._settings.get_int(["plug"])
                self._logger.debug("plug: %s" % self.plug)
	...
	def _shutdown_system(self):
		...
		try:
			import requests
                        requests.get('{self._settings.address}/devicetype/{self._settings.plug}/exec&token={self._settings.token}')
        ...
        def get_update_information(self):
                return dict(
                        ...
                        type="github_release",
                        user="OctoPrint",
                        repo="OctoPrint-Gladys",
                        current=self._plugin_version,

                        # update method: pip w/ dependency links
                        pip="https://github.com/MathieuAndrade/OctoPrint-Gladys/archive/{target_version}.zip"
                )
        )

__plugin_name__ = "Gladys"

def __plugin_load__():
	global __plugin_implementation__
	__plugin_implementation__ = GladysPlugin()
